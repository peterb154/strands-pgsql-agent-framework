...
7. **Migrations: raw SQL files + tiny runner.** Numbered `001_init.sql`, `002_*.sql`. No alembic. **Framework owns 001-099, agents start at 100.** Agent Dockerfile does `COPY migrations/` *after* the framework base image has already copied its own, so framework + agent migrations live side-by-side in `/app/migrations/` and the runner applies them in numeric order.
